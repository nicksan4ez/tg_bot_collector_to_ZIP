...
ARCHIVE_DELAY = float(os.getenv("ARCHIVE_DELAY", "5"))
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for media in media_list:
        ...
        try:
            ...
            raw_name = msg.caption or getattr(media, "file_name", None) or "file"
...
async def _finalize_after_delay(uid: int, ts: float, chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            ...
            await context.bot.send_document(chat_id=chat_id, document=InputFile(zip_path.open("rb")), filename=ZIP_NAME)
